...
function getLatestAssistantFragment() {
  ...
  const candidates =
    collectCandidates(preferredSelectors, { requireMarkdownOrMath: false }) ||
    collectCandidates(fallbackSelectors, { requireMarkdownOrMath: true });
...
function extractTableBlocks(tableElement) {
  ...
  for (const row of rows) {
    const cells = Array.from(row.querySelectorAll("th, td"))
      .map((cell) => collapseWhitespace(cell.innerText || cell.textContent || "").trim())
      .filter(Boolean);
...
function isMathElement(element) {
  const isMathMlElement =
    typeof MathMLElement !== "undefined" && element instanceof MathMLElement;
  ...
  if (
    element.matches(".katex, .katex-display, mjx-container, .MathJax") ||
    element.querySelector(".katex-mathml math, mjx-assistive-mml math")
  ) {
    return true;
  }
